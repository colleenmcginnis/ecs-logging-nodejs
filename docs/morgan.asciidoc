...
Running this script (the full example is {ecs-logging-nodejs-blob}/packages/ecs-morgan-format/examples/express.js[here])
...
"info". For example, running  {ecs-logging-nodejs-blob}/packages/ecs-morgan-format/examples/express.js[examples/express.js]
...
For example, running {ecs-logging-nodejs-blob}/packages/ecs-morgan-format/examples/express-with-apm.js[examples/express-with-apm.js] and `curl -i localhost:3000/` results in a log record with the following:
