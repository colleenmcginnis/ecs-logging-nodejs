...
The {ecs-logging-nodejs-blob}/packages/ecs-pino-format/examples[examples/ directory]
shows sample programs using request and response logging:
{ecs-logging-nodejs-blob}/packages/ecs-pino-format/examples/express-simple.js[with Express],
{ecs-logging-nodejs-blob}/packages/ecs-pino-format/examples/express-with-pino-http.js[with the pino-http middleware package],
etc.
...
For example, running {ecs-logging-nodejs-blob}/packages/ecs-pino-format/examples/http-with-elastic-apm.js[examples/http-with-elastic-apm.js] and `curl -i localhost:3000/` results in a log record with the following:
...
The {ecs-logging-repo}/blob/{branch}/spec[ecs-logging spec]
